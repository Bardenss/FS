...
do
    ...
    local function FormatNumber(n)
        n = math.floor(n or 0) -- Bulatkan ke bawah biar ga ada desimal aneh
    ...
    local function sendExploitWebhook(url, username, embed_data)
        ...
        if typeof(request) == "function" then
            ...
            if success and response and (response.StatusCode == 200 or response.StatusCode == 204) then
                return true, "Sent"
            elseif success and response and response.StatusCode then
    ...
    local function getRarityColor(rarity)
        local r = tostring(rarity or ""):upper()
    ...
    local function shouldNotify(fishRarityUpper, fishMetadata, fishName)
        ...
        if _G.NotifyOnMutation and (fishMetadata and (fishMetadata.Shiny or fishMetadata.VariantId)) then
            return true
        end
    ...
    local function onFishObtained(itemId, metadata, fullData)
        local success, results = pcall(function()
            local dummyItem = { Id = itemId, Metadata = metadata or {} }
            local fishName, fishRarity = GetFishNameAndRarity(dummyItem)
            local fishRarityUpper = (fishRarity or "COMMON"):upper()

            local fishWeight = string.format("%.2fkg", (metadata and metadata.Weight) or 0)
            ...
            local sellPrice = basePrice * ((metadata and metadata.SellMultiplier) or 1)
            ...
            -- Webhook pribadi (Discord)
            local isUserFilterMatch = shouldNotify(fishRarityUpper, metadata or {}, fishName)

            if isWebhookEnabled and WEBHOOK_URL ~= "" and isUserFilterMatch then
                local title_private = string.format("<:TEXTURENOBG:1438662703722790992> BantaiXmarV | Webhook\n\n<a:ChipiChapa:1438661193857503304> New Fish Caught! (%s)", tostring(fishName))

                local embed = {
                    title = title_private,
                    description = string.format("Found by **%s**.", LocalPlayer and (LocalPlayer.DisplayName or LocalPlayer.Name) or "Player"),
                    color = getRarityColor(fishRarityUpper),
                    fields = {
                        { name = "<a:ARROW:1438758883203223605> Fish Name", value = string.format("`%s`", tostring(fishName)), inline = true },
                ...
                if success_send then
                    UpdateWebhookStatus("Webhook Aktif", "Terkirim: " .. tostring(fishName), "check")
                else
                    UpdateWebhookStatus("Webhook Gagal", "Error: " .. tostring(message), "x")
                end
            end

            -- Global webhook via server
            local isGlobalTarget = table.find({"SECRET", "TROPHY", "COLLECTIBLE", "DEV"}, fishRarityUpper)

            if isGlobalTarget then
                local playerName = LocalPlayer and (LocalPlayer.DisplayName or LocalPlayer.Name) or "Player"
                local censoredPlayerName = CensorName(playerName)

                local title_global = string.format("<:TEXTURENOBG:1438662703722790992> BantaiXmarV | Global Tracker\n\n<a:globe:1438758633151266818> GLOBAL CATCH! %s", tostring(fishName))
    ...
    local teswebbut = webhooksec:Button({
        ...
        Callback = function()
            ...
            local testEmbed = {
                ...
                color = 0x00FF00,
                fields = {
                    { name = "Name Player", value = LocalPlayer and (LocalPlayer.DisplayName or LocalPlayer.Name) or "Player", inline = true },
            ...
            if success then
                ...
                WindUI:Notify({ Title = "Test Gagal!", Content = "Cek console (Output) untuk error. " .. tostring(message), Duration = 5, Icon = "x" })
            end
        end
    })
end
